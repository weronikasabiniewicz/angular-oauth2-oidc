...
export class OAuthService extends AuthConfig {
  ...
  protected discoveryDocumentLoadedSubject: Subject<object> = new Subject<
    object
  >();
  ...
  public silentRefresh(
    ...
    this.createLoginUrl(null, null, redirectUri, noPrompt, params).then(url => {
      if (this.config.silentRefreshUseOpenUri) {
        this.config.openUri(url, true);
      } else {
        this.configureIFrameToSilentRefresh(url);
      }
    });
  ...
  private configureIFrameToSilentRefresh(url: string) {
  ...
  public logOut(noRedirectToLogoutUrl = false): void {
    ...
    this.config.openUri(logoutUrl, this.config.logoutUrlIsHidden);
  }
